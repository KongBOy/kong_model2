...
class Col_results_analyzer(Result_analyzer):
    def __init__(self, ana_describe, col_results):
        ...
        self.min_see_file_amount = self.get_min_see_file_amount()
    
    def get_min_see_file_amount(self):
        see_file_amounts = []
    ...
    ########################################################################################################################################
    ### 單一row，同see
    def analyze_col_results_single_see(self, see_num):
        ...
        ### 抓 要顯示的 titles
        c_titles = ["in_img"]
        for result in self.c_results: c_titles.append(result.plot_title)
        c_titles += ["gt_img"]

        ### 抓  要顯示的imgs
        print("doing analyze_col_results_multi_see")
        for go_img in tqdm(range(self.min_see_file_amount)):
            if(go_img >=2):
                epoch = go_img-2
    ...
    def analyze_col_results_all_single_see(self,start_see, see_amount):
        for go_see in range(start_see, start_see + see_amount):
            self.analyze_col_results_single_see(go_see)

    def analyze_col_results_all_single_see_multiprocess(self, core_amount=8, task_amount=32):
        from util import multi_processing_interface
        multi_processing_interface(core_amount=core_amount ,task_amount=task_amount, task=self.analyze_col_results_all_single_see)

    ########################################################################################################################################
    ########################################################################################################################################
    ########################################################################################################################################
    ### 同col同result，同row同see
    def analyze_col_results_multi_see(self, see_nums, save_name):
        ...
        ### 抓 row/col 要顯示的imgs
        print("doing analyze_col_results_multi_see")
        self._draw_col_results_multi_see_multiprocess( see_nums, in_imgs, gt_imgs, r_c_titles, analyze_see_dir, core_amount=8, task_amount=self.min_see_file_amount)
    ...
    def _draw_col_results_multi_see(self, start_img, img_amount, see_nums, in_imgs, gt_imgs, r_c_titles, analyze_see_dir ):
        print("doing analyze_col_results_multi_see")
    ...
    def _draw_col_results_multi_see_multiprocess(self, see_nums, in_imgs, gt_imgs, r_c_titles, analyze_see_dir, core_amount=8, task_amount=100):
        from util import multi_processing_interface
        multi_processing_interface(core_amount=core_amount ,task_amount=task_amount, task=self._draw_col_results_multi_see, task_args=( see_nums, in_imgs, gt_imgs, r_c_titles, analyze_see_dir))



class Row_col_results_analyzer(Result_analyzer):
    def __init__(self, ana_describe, row_col_results):
        super().__init__(self, ana_describe)
            ...
            if(go_img >=2):
                ...
                for row_results in self.r_c_results:
                    ...
                    for result in row_results: ### 抓出一個row的 img 和 title
                        ...
                        c_titles.append(result.plot_title)
