...
def distort_more_like_page(dst_dir, start_index, row, col, write_npy=True):
    ...
    # Check_dir_exist_and_build(Data_Access_Dir + dst_dir + "/"+"distorted_mesh_visuals")
    Check_dir_exist_and_build(Data_Access_Dir + dst_dir + "/" + "move_maps")
    print(Data_Access_Dir + dst_dir + "/" + "move_maps")
    # Check_dir_exist_and_build(Data_Access_Dir + dst_dir + "/"+"distorte_infos")
    ...
    for go_top_curl in range(11, 30 + 1):
        for go_ratio_col_t in range(76, (85 + 1)):
            ...
            if(write_npy) : np.save(Data_Access_Dir + dst_dir + "/" + "move_maps/%06i" % index, combine_move)  ### 把move_map存起來，記得要轉成float32！
...
def distort_just_page(dst_dir, start_index, row, col, repeat=5, write_npy=True):
    ...
    for _ in range(repeat):
        for go_page_curl in range(1, 1 + 55):  ### go_page_curl 最小要1才行喔！
            ...
            if(write_npy) : np.save(Data_Access_Dir + dst_dir + "/" + "move_maps/%06i" % (index), page_move)       ### 把move_map存起來，記得要轉成float32！
            print("%06i process 1 mesh cost time:" % index, "%.3f" % (time.time() - dis_start_time), "total_time:", time_util(time.time() - start_time))
            index += 1
