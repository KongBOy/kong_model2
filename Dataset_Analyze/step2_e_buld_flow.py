...
for move in train_moves:
    np.save(Data_Access_Dir + dst_dir + "/" + "move_maps/%06i_train" % (index), move)  ### 把move_map存起來，記得要轉成float32！
    index += 1

for move in test_moves:
    np.save(Data_Access_Dir + dst_dir + "/" + "move_maps/%06i_test" % (index), move)  ### 把move_map存起來，記得要轉成float32！
    index += 1
