...
def step7b_dis_coord_big_find_ord_valid_mask_and_ord_valid_coord_simulate(dis_coord_big_m, boundary_value=1.0, visual=False, fig=None, ax=None, ax_c=None):
    ...
    for go_r, dis_coord_m_row in enumerate(dis_coord_big_m):
        for go_c, xy_coord in enumerate(dis_coord_m_row):
            ...
            if(-boundary_value <= xy_coord[0] <= boundary_value and -boundary_value <= xy_coord[1] <= boundary_value):
...
def apply_flow(data, flow, visual=False, before_title=None, after_title=None, fig=None, ax=None, ax_c=None):
    ...
    result_t = F.grid_sample(input=data_t, grid=flow_t, align_corners=False)  ### align_corners 看這邊： https://blog.csdn.net/u012348774/article/details/104433061
