...
######################################################################################################################################################################################################
def I_w_Mgt_to_Cy_see(model_obj, phase, index, in_img, in_img_pre, gt_mask_coord, gt_mask_coord_pre, rec_hope=None, exp_obj=None, training=True, see_reset_init=True, postprocess=False, npz_save=False, add_loss=False, bgr2rgb=True):
    '''
    ...
    cy_pre      = model_obj.generator(I_pre_w_M, training=training)
    cy_pre = cy_pre[0].numpy()
    cy = Value_Range_Postprocess_to_01(cy_pre, exp_obj.use_gt_range)
    ...
    Cy_w_Mgt_visual = (cy * gt_mask_pre[0].numpy() * 255).astype(np.uint8)
    ...
    cv2.imwrite(private_write_dir + "/" + "epoch_%04i_u1b2-Cy_w_Mgt.jpg" % current_ep, Cy_w_Mgt_visual)    ### 我覺得不可以直接存npy，因為太大了！但最後為了省麻煩還是存了，相對就減少see的數量來讓總大小變小囉～
