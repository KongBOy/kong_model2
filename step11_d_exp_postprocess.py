...
if(__name__ == "__main__"):
    ...
    for go_result, rec_bm_result in enumerate(rec_bm_results):
        if(go_result == 0):  ### 這個 if 是給 中途不小心中斷，可以從某個指定的 see_num 開始做，要手動把中斷的放第一個result拉
            # rec_bm_result.save_all_single_see_as_matplot_bm_rec_visual_multiprocess(add_loss=False, bgr2rgb=True, print_msg=True)
            rec_bm_result.save_all_single_see_as_matplot_bm_rec_visual(start_index=0, amount=12, add_loss=False, bgr2rgb=True, single_see_multiprocess=True, print_msg=True)
