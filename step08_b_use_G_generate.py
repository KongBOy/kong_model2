...
### 用 網路 生成 影像
def generate_img_results(model_G, in_img_pre):
...
def generate_img_sees(model_G, see_index, in_img_pre, gt_img, epoch=0, result_obj=None, see_reset_init=False):
    rect_back, in_img_back = generate_img_results(model_G, in_img_pre)
...
#######################################################################################################################################
def generate_flow_results(model_G, in_img_pre, training=False):
...
def generate_flow_sees_without_rec(model_G, see_index, in_img_pre, gt_flow, epoch=0, result_obj=None, training=True, see_reset_init=True):
    flow, in_img_back = generate_flow_results(model_G, in_img_pre, training=training)
    gt_flow = gt_flow[0]
