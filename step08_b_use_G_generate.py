...
### 用 網路 生成 影像
def in_img_Generate_out_rec(model_G, _1, in_img_pre, _3, _4, use_gt_range):
...
def in_img_Generate_out_rec_see(model_G, see_index, in_img, in_img_pre, gt_img, _4, rec_hope, epoch=0, result_obj=None, see_reset_init=False):
    '''
    如果之後有需要 in/use_gt_range，可以從result_obj裡面拿喔，就用 result_obj.in/use_gt_range 即可
    '''
    rect_back = in_img_Generate_out_rec(model_G, None, in_img_pre, None, None, result_obj.use_gt_range)
...
######################################################################################################################################################################################################
######################################################################################################################################################################################################
def in_img_Generate_out_flow(model_G, _1, in_img_pre, _3, _4, use_gt_range, training=False):  ### training 這個參數是為了 一開使 用BN ，為了那些exp 還能重現所以才保留，現在用 IN 完全不會使用到他這樣子拉～
...
def in_img_Generate_out_flow_see(model_G, see_index, in_img, in_img_pre, gt_flow, _4, rec_hope, epoch=0, result_obj=None, training=True, see_reset_init=True):
    '''
    如果有需要 in/use_gt_range，可以從result_obj裡面拿喔，就用 result_obj.in/use_gt_range 即可
    '''
    flow           = in_img_Generate_out_flow(model_G, None, in_img_pre, None, None, result_obj.use_gt_range, training=training)
    flow           = flow[0]
    gt_flow        = gt_flow[0]
...
######################################################################################################################################################################################################
######################################################################################################################################################################################################
def in_img_Generate_out_mask(model_G, _1, in_img_pre, _3, _4, use_gt_range, training=False):  ### training 這個參數是為了 一開使 用BN ，為了那些exp 還能重現所以才保留，現在用 IN 完全不會使用到他這樣子拉～
    mask      = model_G(in_img_pre, training=training)
    return mask


def in_img_Generate_out_mask_see(model_G, see_index, in_img, in_img_pre, gt_mask_coord, _4, rec_hope=None, epoch=0, result_obj=None, training=True, see_reset_init=True):
    '''
    如果有需要 in/use_gt_range，可以從result_obj裡面拿喔，就用 result_obj.in/use_gt_range 即可
    '''
    mask           = in_img_Generate_out_mask(model_G, None, in_img_pre, None, None, result_obj.use_gt_range, training=training)
    mask           = mask[0]
...
######################################################################################################################################################################################################
######################################################################################################################################################################################################
def coord_to_flow_by_gt_mask_and_get_visual_img(coord, gt_mask):
    flow        = tf.concat([gt_mask, coord],    axis=-1)  ### channel concate
    flow_visual = flow_or_coord_visual_op(flow).astype(np.uint8)
    return flow, flow_visual
####################################################################################################
def in_img_Generate_out_coord(model_G, _1, in_img_pre, _3, _4, use_gt_range, training=False):  ### training 這個參數是為了 一開使 用BN ，為了那些exp 還能重現所以才保留，現在用 IN 完全不會使用到他這樣子拉～
...
def in_img_Generate_out_coord_to_flow_see(model_G, see_index, in_img, in_img_pre, gt_mask_coord, _4, rec_hope=None, epoch=0, result_obj=None, training=True, see_reset_init=True):
    ...
    coord    = in_img_Generate_out_coord(model_G, None, in_img_pre, None, None, result_obj.use_gt_range, training=training)
    coord    = coord[0]
    gt_mask  = gt_mask_coord[0][0]
...
######################################################################################################################################################################################################
def in_I_with_Mgt_Generate_out_coord(model_G, _1, in_img_pre, _3, gt_mask_coord_pre, use_gt_range, training=False):  ### training 這個參數是為了 一開使 用BN ，為了那些exp 還能重現所以才保留，現在用 IN 完全不會使用到他這樣子拉～
    '''
...
def in_I_with_Mgt_Generate_out_coord_to_flow_see(model_G, see_index, in_img, in_img_pre, gt_mask_coord, gt_mask_coord_pre, rec_hope=None, epoch=0, result_obj=None, training=True, see_reset_init=True):
    ...
    gt_mask_coord[1] 為 coord (1, h, w, 2) 先y 在x
    '''
    coord, I_with_M = in_I_with_Mgt_Generate_out_coord(model_G, None, in_img_pre, None, gt_mask_coord_pre, result_obj.use_gt_range, training=training)
    coord = coord[0]
    gt_mask  = gt_mask_coord[0][0]
    gt_coord = gt_mask_coord[1][0]
    flow,    flow_visual    = coord_to_flow_by_gt_mask_and_get_visual_img(coord,    gt_mask)
