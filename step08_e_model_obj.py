...
class KModel_Flow_Generator_builder(KModel_Unet_builder):
    ...
    def build_flow_unet(self, hid_ch=64, depth_level=7, skip_use_add=False, out_tanh=True, out_ch=3, true_IN=False, concat_Activation=False):
        ### model_part
        ...
        self.kong_model.generator   = Generator(hid_ch=hid_ch, depth_level=depth_level, skip_use_add=skip_use_add, out_tanh=out_tanh, out_ch=out_ch)
        self.kong_model.optimizer_G = tf.keras.optimizers.Adam(2e-4, beta_1=0.5)
...
flow_unet_IN_ch64_sigmoid = KModel_builder().set_model_name(MODEL_NAME.flow_unet).build_flow_unet(hid_ch=64, depth_level=8, skip_use_add=True, out_tanh=False, out_ch=3, true_IN=True)
