...
######################################################################################################################################################################################################
def I_w_Mgt_to_Cx(model_G, _1, in_img_pre, _3, gt_mask_coord_pre, use_gt_range, training=False):  ### training 這個參數是為了 一開使 用BN ，為了那些exp 還能重現所以才保留，現在用 IN 完全不會使用到他這樣子拉～
    '''
    ...
    cx = Value_Range_Postprocess_to_01(cx_pre, use_gt_range)

    I_w_M_visual = (I_pre_w_M[0].numpy() * 255.).astype(np.uint8)
    return cx, I_w_M_visual

def I_w_Mgt_to_Cx_basic_data(model_G, in_img, in_img_pre, gt_mask_coord, gt_mask_coord_pre, rec_hope=None, exp_obj=None, training=True, bgr2rgb=True):
    in_img    = in_img[0].numpy()
    cx, I_w_M_visual = I_w_Mgt_to_Cx(model_G, None, in_img_pre, None, gt_mask_coord_pre, exp_obj.use_gt_range, training=training)
    ...
    return in_img, I_w_M_visual, gt_mask_visual, Cx_visual, gt_cx_visual, rec_hope

def I_w_Mgt_to_Cx_see(model_G, phase, index, in_img, in_img_pre, gt_mask_coord, gt_mask_coord_pre, rec_hope=None, exp_obj=None, training=True, see_reset_init=True, postprocess=False, add_loss=False, bgr2rgb=True):
    ...
    if(current_ep == 0 or see_reset_init):          ### 第一次執行的時候，建立資料夾 和 寫一些 進去資料夾比較好看的東西
        ...
        cv2.imwrite(private_write_dir + "/" + "0a_u1a3-dis_img_w_Mgt(in_img).jpg", I_w_M_visual)                ### 寫一張 in圖進去，進去資料夾時比較好看，0a是為了保證自動排序會放在第一張
