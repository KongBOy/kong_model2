...
class I_w_M_to_W(Use_G_generate):
    ...
    def doing_things(self):
        ...
        if(self.tight_crop is not None):
            ...
            dis_img_pre  , _ = self.tight_crop(dis_img_pre   , Mgt_pre_ord)
            Wgt_w_Mgt    , _ = self.tight_crop(Wgt_w_Mgt     , Mgt_pre_ord)
            Wgt_w_Mgt_pre, _ = self.tight_crop(Wgt_w_Mgt_pre , Mgt_pre_ord)

            ### dis_img 在 tight_crop 完後 不 resize
            tight_crop_resize = self.tight_crop.resize
            self.tight_crop.reset_resize(None)
            dis_img      , _ = self.tight_crop(dis_img, Mgt_pre_ord)
