...
def I_with_Mgt_Generate_C_with_Mgt_to_F_see(model_G, phase, index, in_img, in_img_pre, _3, gt_mask_coord_pre, rec_hope=None, current_ep=0, exp_obj=None, training=True, see_reset_init=True, postprocess=False, add_loss=False, bgr2rgb=True):
    ...
    if(current_ep == 0 or see_reset_init):          ### 第一次執行的時候，建立資料夾 和 寫一些 進去資料夾比較好看的東西
        ...
        Check_dir_exist_and_build(private_write_dir)   ### 建立 model生成的結果 的資料夾
        cv2.imwrite(private_write_dir + "/" + "0a_u1a1-ord_img.jpg",         in_img)
        cv2.imwrite(private_write_dir + "/" + "0a_u1a2-gt_mask.jpg",         Mgt_visual)         ### 寫一張 in圖進去，進去資料夾時比較好看，0a是為了保證自動排序會放在第一張
    ...
    np.save(    private_write_dir + "/" + "epoch_%04i_u1b1-flow"     % current_ep, flow)         ### 我覺得不可以直接存npy，因為太大了！但最後為了省麻煩還是存了，相對就減少see的數量來讓總大小變小囉～
