...
######################################################################################################################################################################################################
def I_with_Mgt_Generate_C(model_obj, _1, in_img_pre, _3, gt_mask_coord_pre, use_gt_range, training=False):  ### training 這個參數是為了 一開使 用BN ，為了那些exp 還能重現所以才保留，現在用 IN 完全不會使用到他這樣子拉～
    '''
    ...
    coord_pre      = model_obj.generator(I_pre_with_M, training=training)
    ...
    return coord, I_w_M_visual

def I_with_Mgt_Generate_C_with_Mgt_to_F_basic_data(model_obj, in_img, in_img_pre, gt_mask_coord_pre, rec_hope=None, exp_obj=None, training=True, bgr2rgb=True):
    in_img    = in_img[0].numpy()
    coord, I_w_M_visual = I_with_Mgt_Generate_C(model_obj, None, in_img_pre, None, gt_mask_coord_pre, exp_obj.use_gt_range, training=training)
    ...
    return in_img, I_w_M_visual, Mgt_visual, flow, flow_visual, gt_flow, gt_flow_visual, Cx_visual, Cy_visual, Cxgt_visual, Cygt_visual, rec_hope

def I_with_Mgt_Gen_C_with_Mgt_to_F_see(model_obj, phase, index, in_img, in_img_pre, _3, gt_mask_coord_pre, rec_hope=None, exp_obj=None, training=True, see_reset_init=True, postprocess=False, npz_save=False, add_loss=False, bgr2rgb=True):
    ...
    '''
    in_img, I_w_M_visual, Mgt_visual, flow, flow_visual, gt_flow, gt_flow_visual, Cx_visual, Cy_visual, Cxgt_visual, Cygt_visual, rec_hope = I_with_Mgt_Generate_C_with_Mgt_to_F_basic_data(model_obj, in_img, in_img_pre, gt_mask_coord_pre, rec_hope=rec_hope, exp_obj=exp_obj, training=training, bgr2rgb=bgr2rgb)
    ...
    if(npz_save is False): np.save            (private_write_dir + "/" + "epoch_%04i_u1b1-flow" % current_ep, flow)         ### 我覺得不可以直接存npy，因為太大了！但最後為了省麻煩還是存了，相對就減少see的數量來讓總大小變小囉～
    if(npz_save is True ): np.savez_compressed(private_write_dir + "/" + "epoch_%04i_u1b1-flow" % current_ep, flow)         ### 我覺得不可以直接存npy，因為太大了！但最後為了省麻煩還是存了，相對就減少see的數量來讓總大小變小囉～
    cv2.imwrite(private_write_dir + "/" + "epoch_%04i_u1b2-flow.jpg" % current_ep, flow_visual)  ### 把 生成的 flow_visual 存進相對應的資料夾
    cv2.imwrite(private_write_dir + "/" + "epoch_%04i_u1b3-Cx.jpg"   % current_ep, Cx_visual)    ### 我覺得不可以直接存npy，因為太大了！但最後為了省麻煩還是存了，相對就減少see的數量來讓總大小變小囉～
    cv2.imwrite(private_write_dir + "/" + "epoch_%04i_u1b4-Cy.jpg"   % current_ep, Cy_visual)    ### 我覺得不可以直接存npy，因為太大了！但最後為了省麻煩還是存了，相對就減少see的數量來讓總大小變小囉～
