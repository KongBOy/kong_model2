...
class See_flow_visual(See_info):
    ...
    # def get_flow_info(self):
        ...
        # self.flow_epoch_jpg_read_paths = [self.see_read_dir + "/" + epoch_jpg_name for epoch_jpg_name in self.see_flow_epoch_jpg_names]  ### 沒有 write_paths， 同上
    ...
    def _Draw_matplot_visual(self, epoch, add_loss=False, bgr2rgb=False):
        ...
        img    = cv2.imread(self.flow_epoch_jpg_read_paths[epoch])  ### see資料夾 內的影像 該epoch產生的影像 讀出來
    ...
    def Save_as_matplot_visual(self, add_loss=False, bgr2rgb=False, single_see_core_amount=8, see_print_msg=False):
        ...
        if(single_see_core_amount == 1):  ### single_see_core_amount 大於1 代表 單核心跑， 就重新導向 最原始的function囉 把 see內的任務 依序完成！
            self._draw_matplot_visual_after_train(0, self.flow_epoch_jpg_amount, add_loss=add_loss, bgr2rgb=bgr2rgb)
            ...
            multi_processing_interface(core_amount=single_see_core_amount, task_amount=self.flow_epoch_jpg_amount, task=self._draw_matplot_visual_after_train, task_args=[add_loss, bgr2rgb], print_msg=see_print_msg)
    ...
    def _draw_matplot_visual_after_train(self, start_epoch, epoch_amount, add_loss=False, bgr2rgb=False):
        ...
        for go_epoch in tqdm(range(start_epoch, start_epoch + epoch_amount)):
            ...
            if(add_loss)   : single_row_imgs.Draw_ax_loss_after_train(single_row_imgs.ax[-1, 1], self.see_read_dir + "/../logs", go_epoch, min_epochs=self.flow_epoch_jpg_amount, ylim=0.04)
