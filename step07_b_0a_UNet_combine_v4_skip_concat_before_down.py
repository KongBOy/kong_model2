...
from step07_a_unet_component_v4 import Conv_Blocks, UNet_down, UNet_up
...
class Generator(tf.keras.models.Model):
    ...
    def call(self, input_tensor, training=None):
        ...
        #####################################################
        ### Down top
        x, skip, x_before_down = self.d_top(input_tensor)
        if(self.concat_before_down is True or self.out_conv_block is True):  skips.append(x_before_down)
        else: skips.append(skip)

        if(self.debug):
            print(f"{self.d_top.name} x_before_down: {x_before_down[0, 0, 0, :3]}")  ### debug 用
            print(f"{self.d_top.name} skip: {skip[0, 0, 0, :3]}")  ### debug 用

        ### Down middle
        for name, d_middle in self.d_middles.items():
            x, skip, x_before_down = d_middle(x)
            if  (self.concat_before_down is False): skips.append(skip)
        ...
        x_bottle, skip, x_before_down = self.d_bottle(x)  ### down 的 bottle沒有 skip
