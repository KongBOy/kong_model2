from step1_c_move_map_visual import get_db_all_move,method2
import numpy as np 
import cv2
...
def search_mask_have_hole(dis_msk, hole_size=1):
    ...
    check_foreground_ok = check_foreground_size**2*0.8 ### 要超過check_foreground_ok才算事前景，沒超過就算背景
...
pag_msk = np.zeros(shape=(dis_h,dis_w),   dtype=np.float64)
...
####################################################################################################################
#### 扭曲影像 空洞的地方補起來
while(search_mask_have_hole(dis_msk) > 0):
    ...
    for go_row in range(dis_h):
        for go_col in range(dis_w):
            if(dis_msk_ref[go_row,go_col]==0 and dis_msk_ref[go_row-1:go_row+2,go_col-1:go_col+2].sum() >=6 ):
                # print(go_row,go_col)
                l,r,t,d = get_l_r_t_d(3)
                msk_sum = dis_msk_ref[go_row-l:go_row+r,go_col-t:go_col+d].sum()
