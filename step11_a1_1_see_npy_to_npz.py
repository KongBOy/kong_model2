...
from step0_access_path import  CORE_AMOUNT_NPY_TO_NPZ
...
class See_npy_to_npz(See_info):
    def __init__(self, result_obj, see_name):
        super(See_npy_to_npz, self).__init__(result_obj, see_name)
        """
    ...
    def get_npz_info(self):
        ...
        if(self.flow_gt_npz_path is None): self.flow_gt_npz_path = self.get_path_savely(self.npz_read_dir, certain_word="Fgt",     certain_ext=".npz")
        ...
        if(len(self.npz_epoch_flow_names) == 0): self.npz_epoch_flow_names = get_dir_certain_file_names(self.npz_read_dir, certain_word="epoch", certain_word2="F.npz",       certain_ext=".npz")
        if(len(self.npz_epoch_flow_names) == 0): self.npz_epoch_flow_names = get_dir_certain_file_names(self.npz_read_dir, certain_word="epoch", certain_word2="F_w_Mgt.npz", certain_ext=".npz")
