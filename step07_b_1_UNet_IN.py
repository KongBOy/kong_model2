...
class Generator(tf.keras.models.Model):
    def __init__(self, hid_ch=64, depth_level=7, no_concat_layer=0, use_bias=True, skip_use_add=False, skip_use_cSE=False, skip_use_sSE=False, skip_use_scSE=False, skip_use_cnn=False, skip_cnn_k=3, skip_use_Acti=None, out_tanh=True, out_ch=3, **kwargs):
        """
        ...
        self.conv1 = Conv2D(min(hid_ch * 2**0, 512), kernel_size=(4, 4), strides=(2, 2), padding="same", use_bias=use_bias, name="conv1")  #,bias=False) ### in_channel:3

        if(self.depth_level >= 2):
            self.lrelu2 = LeakyReLU(alpha=0.2, name="lrelu2")
            self.conv2  = Conv2D(min(hid_ch * 2**1, 512), kernel_size=(4, 4), strides=(2, 2), padding="same", use_bias=use_bias, name="conv2")  #,bias=False) ### in_channel:64
            ...
            if(self.no_concat_layer < 2):
                if(self.skip_use_cSE):  self.skip_cSE2  = cSE(min(hid_ch * 2**0, 512), ratio=min(hid_ch * 2**1, 512) // 32, name="skip_cSE2")
                if(self.skip_use_sSE):  self.skip_sSE2  = sSE(name="skip_sSE2")
                if(self.skip_use_scSE): self.skip_scSE2 = scSE(min(hid_ch * 2**0, 512), ratio=min(hid_ch * 2**1, 512) // 32, name="skip_scSE2")
                if(self.skip_use_cnn):  self.skip_cnn2  = Conv2D(min(hid_ch * 2**1, 512), kernel_size=(skip_cnn_k, skip_cnn_k), strides=(1, 1), padding="same", use_bias=use_bias, name="skip_cnn2")
        ...
        if(self.depth_level >= 3):
            ...
            self.conv3  = Conv2D(min(hid_ch * 2**2, 512), kernel_size=(4, 4), strides=(2, 2), padding="same", use_bias=use_bias, name="conv3")  #,bias=False) ### in_channel:128
            ...
            if(self.no_concat_layer < 3):
                if(self.skip_use_cSE):  self.skip_cSE3  = cSE(min(hid_ch * 2**1, 512), ratio=min(hid_ch * 2**2, 512) // 32, name="skip_cSE3")
                if(self.skip_use_sSE):  self.skip_sSE3  = sSE(name="skip_sSE3")
                if(self.skip_use_scSE): self.skip_scSE3 = scSE(min(hid_ch * 2**1, 512), ratio=min(hid_ch * 2**2, 512) // 32, name="skip_scSE3")
                if(self.skip_use_cnn):  self.skip_cnn3  = Conv2D(min(hid_ch * 2**2, 512), kernel_size=(skip_cnn_k, skip_cnn_k), strides=(1, 1), padding="same", use_bias=use_bias, name="skip_cnn3")
        ...
        if(self.depth_level >= 4):
            ...
            self.conv4  = Conv2D(min(hid_ch * 2**3, 512), kernel_size=(4, 4), strides=(2, 2), padding="same", use_bias=use_bias, name="conv4")  #,bias=False) ### in_channel:256
            ...
            if(self.no_concat_layer < 4):
                if(self.skip_use_cSE):  self.skip_cSE4  = cSE(min(hid_ch * 2**2, 512), ratio=min(hid_ch * 2**3, 512) // 32, name="skip_cSE4")
                if(self.skip_use_sSE):  self.skip_sSE4  = sSE(name="skip_sSE4")
                if(self.skip_use_scSE): self.skip_scSE4 = scSE(min(hid_ch * 2**2, 512), ratio=min(hid_ch * 2**3, 512) // 32, name="skip_scSE4")
                if(self.skip_use_cnn):  self.skip_cnn4  = Conv2D(min(hid_ch * 2**3, 512), kernel_size=(skip_cnn_k, skip_cnn_k), strides=(1, 1), padding="same", use_bias=use_bias, name="skip_cnn4")
        ...
        if(self.depth_level >= 5):
            ...
            self.conv5  = Conv2D(min(hid_ch * 2**4, 512), kernel_size=(4, 4), strides=(2, 2), padding="same", use_bias=use_bias, name="conv5")  #,bias=False) ### in_channel:512
            ...
            if(self.no_concat_layer < 5):
                if(self.skip_use_cSE):  self.skip_cSE5  = cSE(min(hid_ch * 2**3, 512), ratio=min(hid_ch * 2**4, 512) // 32, name="skip_cSE5")
                if(self.skip_use_sSE):  self.skip_sSE5  = sSE(name="skip_sSE5")
                if(self.skip_use_scSE): self.skip_scSE5 = scSE(min(hid_ch * 2**3, 512), ratio=min(hid_ch * 2**4, 512) // 32, name="skip_scSE5")
                if(self.skip_use_cnn):  self.skip_cnn5  = Conv2D(min(hid_ch * 2**4, 512), kernel_size=(skip_cnn_k, skip_cnn_k), strides=(1, 1), padding="same", use_bias=use_bias, name="skip_cnn5")
        ...
        if(self.depth_level >= 6):
            ...
            self.conv6  = Conv2D(min(hid_ch * 2**5, 512), kernel_size=(4, 4), strides=(2, 2), padding="same", use_bias=use_bias, name="conv6")  #,bias=False) ### in_channel:512
            ...
            if(self.no_concat_layer < 6):
                if(self.skip_use_cSE):  self.skip_cSE6  = cSE(min(hid_ch * 2**4, 512), ratio=min(hid_ch * 2**5, 512) // 32, name="skip_cSE6")
                if(self.skip_use_sSE):  self.skip_sSE6  = sSE(name="skip_sSE6")
                if(self.skip_use_scSE): self.skip_scSE6 = scSE(min(hid_ch * 2**4, 512), ratio=min(hid_ch * 2**5, 512) // 32, name="skip_scSE6")
                if(self.skip_use_cnn):  self.skip_cnn6  = Conv2D(min(hid_ch * 2**5, 512), kernel_size=(skip_cnn_k, skip_cnn_k), strides=(1, 1), padding="same", use_bias=use_bias, name="skip_cnn6")
        ...
        if(self.depth_level >= 7):
            ...
            self.conv7   = Conv2D(min(hid_ch * 2**6, 512), kernel_size=(4, 4), strides=(2, 2), padding="same", use_bias=use_bias, name="conv7")  #,bias=False) ### in_channel:512
            ...
            if(self.no_concat_layer < 7):
                if(self.skip_use_cSE):  self.skip_cSE7  = cSE(min(hid_ch * 2**5, 512), ratio=min(hid_ch * 2**6, 512) // 32, name="skip_cSE7")
                if(self.skip_use_sSE):  self.skip_sSE7  = sSE(name="skip_sSE7")
                if(self.skip_use_scSE): self.skip_scSE7 = scSE(min(hid_ch * 2**5, 512), ratio=min(hid_ch * 2**6, 512) // 32, name="skip_scSE7")
                if(self.skip_use_cnn):  self.skip_cnn7  = Conv2D(min(hid_ch * 2**6, 512), kernel_size=(skip_cnn_k, skip_cnn_k), strides=(1, 1), padding="same", use_bias=use_bias, name="skip_cnn7")
        ...
        if(self.depth_level >= 8):
            ...
            self.conv8   = Conv2D(min(hid_ch * 2**7, 512), kernel_size=(4, 4), strides=(2, 2), padding="same", use_bias=use_bias, name="conv8")  #,bias=False) ### in_channel:512
            ...
            if(self.no_concat_layer < 8):
                if(self.skip_use_cSE):  self.skip_cSE8  = cSE(min(hid_ch * 2**6, 512), ratio=min(hid_ch * 2**7, 512) // 32, name="skip_cSE8")
                if(self.skip_use_sSE):  self.skip_sSE8  = sSE(name="skip_sSE8")
                if(self.skip_use_scSE): self.skip_scSE8 = scSE(min(hid_ch * 2**6, 512), ratio=min(hid_ch * 2**7, 512) // 32, name="skip_scSE8")
                if(self.skip_use_cnn):  self.skip_cnn8  = Conv2D(min(hid_ch * 2**7, 512), kernel_size=(skip_cnn_k, skip_cnn_k), strides=(1, 1), padding="same", use_bias=use_bias, name="skip_cnn8")
        ...
        if(self.depth_level >= 9):
            ...
            self.conv9   = Conv2D(min(hid_ch * 2**8, 512), kernel_size=(4, 4), strides=(2, 2), padding="same", use_bias=use_bias, name="conv9")  #,bias=False) ### in_channel:512
            if(self.no_concat_layer < 9):
                if(self.skip_use_cSE):  self.skip_cSE9  = cSE(min(hid_ch * 2**7, 512), ratio=min(hid_ch * 2**8, 512) // 32, name="skip_cSE9")
                if(self.skip_use_sSE):  self.skip_sSE9  = sSE(name="skip_sSE9")
                if(self.skip_use_scSE): self.skip_scSE9 = scSE(min(hid_ch * 2**7, 512), ratio=min(hid_ch * 2**8, 512) // 32, name="skip_scSE9")
                if(self.skip_use_cnn):  self.skip_cnn9  = Conv2D(min(hid_ch * 2**8, 512), kernel_size=(skip_cnn_k, skip_cnn_k), strides=(1, 1), padding="same", use_bias=use_bias, name="skip_cnn9")
        ...
        if(self.depth_level >= 9):
            ...
            self.conv9t  = Conv2DTranspose(min(hid_ch * 2**7, 512), kernel_size=(4, 4), strides=(2, 2), padding="same", use_bias=use_bias, name="conv9t")  #,bias=False) ### in_channel:512
        ...
        if(self.depth_level >= 8):
            ...
            self.conv8t  = Conv2DTranspose(min(hid_ch * 2**6, 512), kernel_size=(4, 4), strides=(2, 2), padding="same", use_bias=use_bias, name="conv8t")  #,bias=False) ### in_channel:512
        ...
        if(self.depth_level >= 7):
            ...
            self.conv7t  = Conv2DTranspose(min(hid_ch * 2**5, 512), kernel_size=(4, 4), strides=(2, 2), padding="same", use_bias=use_bias, name="conv7t")  #,bias=False) ### in_channel:512
        ...
        if(self.depth_level >= 6):
            ...
            self.conv6t  = Conv2DTranspose(min(hid_ch * 2**4, 512), kernel_size=(4, 4), strides=(2, 2), padding="same", use_bias=use_bias, name="conv6t")  #,bias=False) ### in_channel:1024
        ...
        if(self.depth_level >= 5):
            ...
            self.conv5t  = Conv2DTranspose(min(hid_ch * 2**3, 512), kernel_size=(4, 4), strides=(2, 2), padding="same", use_bias=use_bias, name="conv5t")  #,bias=False) ### in_channel:1024
        ...
        if(self.depth_level >= 4):
            ...
            self.conv4t  = Conv2DTranspose(min(hid_ch * 2**2, 512), kernel_size=(4, 4), strides=(2, 2), padding="same", use_bias=use_bias, name="conv4t")  #,bias=False) ### in_channel:1024
        ...
        if(self.depth_level >= 3):
            ...
            self.conv3t  = Conv2DTranspose(min(hid_ch * 2**1, 512), kernel_size=(4, 4), strides=(2, 2), padding="same", use_bias=use_bias, name="conv3t")  #,bias=False) ### in_channel:512
        ...
        if(self.depth_level >= 2):
            ...
            self.conv2t  = Conv2DTranspose(min(hid_ch * 2**0, 512), kernel_size=(4, 4), strides=(2, 2), padding="same", use_bias=use_bias, name="conv2t")  #,bias=False) ### in_channel:256
        ...
        self.conv1t = Conv2DTranspose(out_ch, kernel_size=(4, 4), strides=(2, 2), padding="same", use_bias=use_bias, name="conv1t")  ### in_channel:128
...
if(__name__ == "__main__"):
    ...
    flow_unet_IN_L7_ch64_skip_use_cnn1_USErelu    = KModel_builder().set_model_name(MODEL_NAME.flow_unet).hook_build_and_gen_op(hid_ch=64, skip_use_cnn=True, skip_cnn_k=1, skip_use_Acti=tf.nn.relu, true_IN=True).build()
    flow_unet_IN_L7_ch64_skip_use_cnn1_USEsigmoid = KModel_builder().set_model_name(MODEL_NAME.flow_unet).hook_build_and_gen_op(hid_ch=64, skip_use_cnn=False, skip_cnn_k=1, skip_use_Acti=tf.nn.sigmoid, true_IN=True).build()
    flow_unet_IN_L7_ch64_skip_use_cSE             = KModel_builder().set_model_name(MODEL_NAME.flow_unet).hook_build_and_gen_op(hid_ch=64, skip_use_cSE=True, true_IN=True)
    flow_unet_IN_L7_ch64_skip_use_sSE             = KModel_builder().set_model_name(MODEL_NAME.flow_unet).hook_build_and_gen_op(hid_ch=64, skip_use_sSE=True, true_IN=True)
    flow_unet_IN_L7_ch64_skip_use_scSE            = KModel_builder().set_model_name(MODEL_NAME.flow_unet).hook_build_and_gen_op(hid_ch=64, skip_use_scSE=True, true_IN=True)
    flow_unet_IN_ch64_cnnNoBias                   = KModel_builder().set_model_name(MODEL_NAME.flow_unet).hook_build_and_gen_op(hid_ch=64, true_IN=True, use_bias=False)
