...
test_db       = unet_imgs_test_db 
test_label_db = gt_imgs_test_db
...
for i, (test_input, test_label) in enumerate(zip(test_db.take(200), test_label_db.take(200))): 
    ...
    ### 圖. unet_img
    unet_img  = test_input[0].numpy() 
    unet_img = (unet_img+1)*127.5
    unet_img = unet_img.astype(np.uint8)
    ax[0].imshow(unet_img)
    ax[0].set_title("unet_img")

    ### 圖. rect2恢復unet_img的結果
