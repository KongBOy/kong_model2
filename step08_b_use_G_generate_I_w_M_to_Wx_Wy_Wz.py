...
def use_model(model_obj, _1, in_img_pre, _3, Wgt_w_Mgt_pre, use_gt_range, training=False):  ### training 這個參數是為了 一開使 用BN ，為了那些exp 還能重現所以才保留，現在用 IN 完全不會使用到他這樣子拉～
    ...
    Wz_pre, Wy_pre, Wx_pre = model_obj.generator(I_pre_with_M_pre, training=training)
...
def I_w_M_Gen_Wx_Wy_Wz_to_W_see(model_obj, phase, index, in_img, in_img_pre, _3, Wgt_w_Mgt_pre, rec_hope=None, exp_obj=None, training=True, see_reset_init=True, postprocess=False, npz_save=False, add_loss=False, bgr2rgb=True):
