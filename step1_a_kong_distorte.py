...
from util import distrore_info_log
...
def distorte_rand(row=40, col=30, index=0):
    ...
    for _ in range(distore_times):
        ...
        ### 紀錄扭曲參數
        if(curve_type == "fold"):    
            distrore_info_log("step1_result/distorte_info", index, row, col, distore_times, x, y, move_x, move_y, curve_type, alpha_fold, )
        elif curve_type == "curl":
            distrore_info_log("step1_result/distorte_info", index, row, col, distore_times, x, y, move_x, move_y, curve_type, alpha_curl, )
...
#    plt.show()
    ##############################################################################################
    ### 隨機生成
    Check_dir_exist_and_build_new_dir("step1_result/distorted_mesh")
    Check_dir_exist_and_build_new_dir("step1_result/move_map")
    Check_dir_exist_and_build_new_dir("step1_result/distorte_info")
    ...
    for index in range(100):
        ...
        # plt.show()
        plt.savefig("step1_result/distorted_mesh/%06i.png"%index)
        plt.close()
        ...
        np.save("step1_result/move_map/%06i"%index,result_move_map.astype(np.float32))
