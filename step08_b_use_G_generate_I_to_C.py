...
######################################################################################################################################################################################################
def I_Generate_C(model_obj, _1, in_img_pre, _3, _4, use_gt_range, training=False):  ### training 這個參數是為了 一開使 用BN ，為了那些exp 還能重現所以才保留，現在用 IN 完全不會使用到他這樣子拉～
    coord_pre = model_obj.generator(in_img_pre, training=training)
    coord_pre = coord_pre[0].numpy()
    coord = Value_Range_Postprocess_to_01(coord_pre, use_gt_range)
    return coord

def I_Gen_C_w_Mgt_to_F_basic_data(model_obj, in_img, in_img_pre, gt_mask_coord_pre, rec_hope=None, exp_obj=None, training=True, bgr2rgb=True):
    '''
    ...
    coord    = I_Generate_C(model_obj, None, in_img_pre, None, None, exp_obj.use_gt_range, training=training)
...
def I_Gen_C_w_Mgt_to_F_see(model_obj, phase, index, in_img, in_img_pre, _3, gt_mask_coord_pre, rec_hope=None, exp_obj=None, training=True, see_reset_init=True, postprocess=False, npz_save=False, add_loss=False, bgr2rgb=True):
    ...
    bgr2rgb： tf2 讀出來是 rgb， 但 cv2 存圖是bgr， 所以此狀況記得要轉一下ch 把 bgr2rgb設True！
    '''
    in_img, gt_mask, gt_mask_visual, gt_flow_visual, gt_flow, rec_hope, flow, flow_visual, Cx_visual, Cy_visual, Cxgt_visual, Cygt_visual = I_Gen_C_w_Mgt_to_F_basic_data(model_obj, in_img, in_img_pre, gt_mask_coord_pre, rec_hope, exp_obj=exp_obj, training=training, bgr2rgb=bgr2rgb)
